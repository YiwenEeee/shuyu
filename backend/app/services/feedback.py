...
def _personalized(source_content: str, candidate_text: str, match_score: float, tier: str) -> str:
    vibe = "几乎说到一块去了，直击灵魂" if tier == "S" else "同书同感，但角度略有不同"
    system = "你是一个懂读书、擅长用文字连接同频者的推荐官。"
    prompt = (
        f"用户 A 的读书笔记：\n{source_content[:200]}\n\n"
        f"用户 B 的最相近摘录（匹配度 {match_score:.0f}%）：\n{candidate_text[:200]}\n\n"
        f"请用 30 字以内，为 A 写一段推荐语，介绍 TA 与 B 的『{vibe}』。"
        "语气真诚、有温度，不要用『亲』『您』等称呼，不要堆砌感叹号。只输出推荐语本身。"
    )
...
def build_feedback(
    ...
    if tier in ("S", "A"):
        try:
            return _personalized(source_content, candidate_text, match_score, tier)
        except LLMError:
            return None
